...
const dmi_table_spec_t dmi_ipmi_device_table =
{
    ...
    .handlers    = {
        .decode = (dmi_table_decode_fn_t)dmi_ipmi_device_decode,
        .free   = (dmi_table_free_fn_t)dmi_ipmi_device_free
    }
};
...
dmi_ipmi_device_t *dmi_ipmi_device_decode(const dmi_table_t *table, dmi_version_t *plevel)
{
    ...
    data = dmi_cast(data, dmi_table_data(table, DMI_TYPE_IPMI_DEVICE));
    if (!data)
        return nullptr;
