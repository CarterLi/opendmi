...
static const dmi_name_t dmi_memory_module_voltage_names[] =
{
    {
        .id   = 0,
        .code = "5v",
...
const dmi_table_spec_t dmi_memory_controller_table =
{
    ...
    .handlers   = {
        .decode = (dmi_table_decode_fn_t)dmi_memory_controller_decode,
        .free   = (dmi_table_free_fn_t)dmi_memory_controller_free
    }
};
...
dmi_memory_controller_t *dmi_memory_controller_decode(const dmi_table_t *table, dmi_version_t *plevel)
{
    ...
    data = dmi_cast(data, dmi_table_data(table, DMI_TYPE_MEMORY_CONTROLLER));
    if (!data)
        return nullptr;
    ...
    info->module_handles = dmi_alloc_array(table->context, sizeof(dmi_handle_t),
                                           info->slot_count);
    ...
    const dmi_data_t *extra_start = table->data + sizeof(*data) +
